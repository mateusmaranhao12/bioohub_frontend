<template>
    <div v-if="modoTela === 'celular' || modoTela === 'computador'">
        <div :class="{ 'container': modoTela === 'computador', 'container-celular mt-5 mb-5': modoTela === 'celular' }"
            ...
            <div v-else class="row">
                ...
                <div
                    ...
                    <div class="d-flex flex-wrap gap-3 justify-content-between">
                        ...
                        <!--Inserir nota-->
                        <InserirNotas :notaSalva="notaSalva" :editandoNota="editandoNota" :textareaStyle="textareaStyle"
                            @salvarNota="salvarNota" @salvarEdicaoNota="salvarEdicaoNota" @editarNota="editarNota"
                            @removerNota="removerNota" :limparTextarea="limparTextarea" />
                        ...
                        <!--Inserir imagens-->
                        <InserirImagens :imagemSelecionada="imagemSelecionada" :imagemUrl="imagemUrl" :loading="loading"
                            @carregarImagem="carregarImagem" @abrirSeletorImagem="abrirSeletorImagem"
            ...
            <MenuConfig v-if="!carregando && modoTela === 'computador'" :email="email" :usuario="usuario"
                @logout="fazerLogout" />
...
export default class PaginaUsuario extends Vue {
            ...
            .then((response) => {
                ...
                if (dados.imagens_footer && dados.imagens_footer.length > 0) {
                    this.imagensFooter = dados.imagens_footer.map((imagem: any) => {
                        return {
                            id: imagem.id, // Aqui é importante garantir que você esteja pegando o ID correto
                            imagem: `data:image/jpeg;base64,${imagem.imagem}`,
                            usuario_id: imagem.usuario_id // Certifique-se de incluir o usuario_id, se necessário
                        };
                    });
                }
    ...
    // Salvar nota
    public async salvarNota(nota: string) {
        const userId = sessionStorage.getItem('user_id');

        const notaObj = {
            usuario_id: userId,
            nota: nota  // Use a nota recebida do evento
        };
    ...
    public async removerNota(id: number) {
        ...
        try {
            ...
            if (response.data.success) {
                ...
                this.notaSalva = null;
                this.editandoNota = false;
                this.limparTextarea = true;  // Indicar que o textarea deve ser limpo

                // Após um pequeno intervalo, redefina 'limparTextarea' para false para evitar futuras limpezas inesperadas
                setTimeout(() => {
                    this.limparTextarea = false;
                }, 100);
            } else {
                this.mostrarMensagemAlerta('fa-solid fa-exclamation-circle', response.data.message, 'alert-error');
            }
    ...
    public async salvarEdicaoNota(nota: string) {
        const userId = sessionStorage.getItem('user_id');
    ...
    public async handleImagemFooterSelecionada(file: File) {
        ...
        try {
            ...
            if (response.data.mensagem === 'Imagem inserida com sucesso') {
                ...
                console.log('ID da imagem adicionada: ', response.data.id)
                // Adiciona a imagem ao array (não mudamos o ID aqui, apenas adicionamos)
    ...
    public async removerImagemFooter(index: number) {
        const usuario_id = sessionStorage.getItem('user_id');  // Obtém o ID do usuário
        ...
        if (index === undefined || index < 0 || index >= this.imagensFooter.length) {
            console.log('Índice inválido:', index);  // Log para verificar o índice
            this.mostrarMensagemAlerta('fa-solid fa-exclamation-circle', 'Índice inválido', 'alert-error');
            return;
        }

        const imagem = this.imagensFooter[index];  // Obtemos a imagem pelo índice
        const imagemId = imagem.id;  // Pegamos o ID da imagem a partir do índice

        console.log(`Remover imagem: Usuario ID: ${usuario_id}, Imagem ID: ${imagemId}`);

        try {
            // Verifica se as imagens foram carregadas corretamente
            if (!this.imagensFooter || this.imagensFooter.length === 0) {
                this.mostrarMensagemAlerta('fa-solid fa-exclamation-circle', 'Nenhuma imagem carregada', 'alert-error');
                return;
            }
            ...
            if (response.data.mensagem === 'Imagem removida com sucesso') {
                // Remove a imagem localmente usando o index
                this.imagensFooter.splice(index, 1);
                this.mostrarMensagemAlerta('fa-solid fa-check', 'Imagem removida com sucesso', 'alert-sucesso');
                // Recarregar as imagens do backend após remoção
                this.carregarDadosBackend(this.username);  // Recarregar os dados após a remoção
            } else {
